...
(define-syntax (#%app stx)
  (syntax-parse stx #:literals (^ neg ¬)
    [(_ (a ^ b) ...)
     #'(normalize (list (cons a b) ...))]
    [(_ neg rest ...)
     #'(append (list (cons -1 1)) (rest ...))]
    [(_ ¬ rest ...)
     #'(append (list (cons -1 1)) (rest ...))]
...
(define normalize
  (λ (ls)
    (let ([res (normalize-helper ls)])
      (match res
        ['() '()]
        ['((1 . 1) . _) res]
        ['((-1 . 1) (1 . _) . _) res]
        ['((-1 . 1) . _) (cons (cons -1 1) (cons (cons 1 1) (cdr res)))]
        [_ (cons (cons 1 1) res)]))))
(define normalize-helper
  (λ (ls)
    (cond
      [(null? ls) '()]
      [(equal? 1 (caar ls)) (cons (cons 1 1) (cdr ls))]
